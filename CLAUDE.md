...
CEO is Charles's personal AI assistant ("Jarvis but called CEO"). It runs a Python/FastAPI server on the Desktop and a React Native (Expo) mobile app. The LLM brain is **Gemini 2.0 Flash** (not Claude — Claude tokens are reserved for development via Claude Code CLI).
